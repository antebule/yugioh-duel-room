...
10. **Empty state + zone browser** — `EmptyStateOverlay.vue` (centered CTA over empty playmat, doubles as drop target); `ModalShell.vue` + `ZoneBrowserModal.vue` (click GY/Banished/Extra/Deck → grid of all cards, contextual menu reused). → empty-state CTA shows pre-deck; stacked-zone clicks open live grid with working context menus.
11. **Info bars + LP controls** — `LifePointsControl.vue`, `PlayerInfoBar.vue` (LP +/- + phase/turn indicator), `OpponentInfoBar.vue` (label only, no LP). → LP +/-/Set updates state + log; phase/turn render and update reactively.
12. **Controls bar + Phase 4 stubs + polish** — `ControlsBar.vue`: Roll Dice / Coin Toss / End Phase (cycles DP→SP→M1→BP→M2→EP→ next turn DP) / Reset Duel (returns cards to deck, reshuffles, clears log). Undo + Save Replay disabled with `ToolTip` "Coming in Phase 4". Lint + format + 100-card perf check.
